...
namespace yae
{
  ...
  struct Dismiss : public ClickableItem
  {
    ...
    {
      yae::queue_call(item_, &FrameCropItem::emit_done);
      return true;
    }
  ...
  {
    RegionSelect(FrameCropItem & item,
                 DonutRect & rect,
  ...
  //
  FrameCropItem::FrameCropItem(const char * id, ItemView & view):
    QObject(),
    Item(id),
    view_(view)
  {
    ...
    // setup mouse trap to prevent unintended click-through to mainItem:
    MouseTrap & mouseTrap = root.addNew<MouseTrap>("mouse_trap");
    mouseTrap.onScroll_ = false;
    mouseTrap.anchors_.fill(root);

    uncropped_.reset(new CanvasRendererItem("uncropped", view_));
    CanvasRendererItem & uncropped = root.add(uncropped_);
  ...
  //----------------------------------------------------------------
  // FrameCropItem::processMouseTracking
  //
  bool
  FrameCropItem::processMouseTracking(const TVec2D & mousePt)
  {
    if (!view_.isEnabled())
    {
      return false;
    ...
    DonutRect & donut = uncropped_->get<DonutRect>("donut");
    view_.requestUncache(&donut);

    return true;
 }

  //----------------------------------------------------------------
  // FrameCropItem::processKeyEvent
  //
  bool
  FrameCropItem::processKeyEvent(Canvas * canvas, QKeyEvent * e)
  {
    e->ignore();
  ...
  //----------------------------------------------------------------
  // FrameCropItem::setCrop
  //
  void
  FrameCropItem::setCrop(const TVideoFramePtr & frame, const TCropFrame & crop)
  {
  ...
  //
  void
  FrameCropItem::setCrop(const Segment & xCrop, const Segment & yCrop)
  {
    CanvasRendererItem & uncropped = *uncropped_;
